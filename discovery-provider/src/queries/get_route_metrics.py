...
def get_aggregate_route_metrics(time_range, bucket_size):
    ...
    with db.scoped_session() as session:
        ...
        twelve_months_ago = today - timedelta(days=365) # do we care about leap years?
        ...
        if time_range == 'week':
            if bucket_size == 'day':
                unique_counts = (
                    ...
                    .filter(seven_days_ago <= DailyUniqueUsersMetrics.timestamp)# < today)
                    .filter(DailyUniqueUsersMetrics.timestamp < today)
                    .all()
                )
                unique_count_records = ft.reduce(lambda acc, curr: acc.update({str(curr[0]): curr[1]}) or acc, unique_counts, {})
                ...
                total_count_records = ft.reduce(lambda acc, curr: acc.update({str(curr[0]): curr[1]}) or acc, total_counts, {})
                ...
                for timestamp, unique_count in unique_count_records.items():
                    if timestamp in total_count_records:
                        metrics.append({
                            ...
                            'count': total_count_records[timestamp]
                        })
                return metrics
            else:
            ...
            if bucket_size == 'month':
                unique_counts = (
                    ...
                    .filter(twelve_months_ago <= MonthlyUniqueUsersMetrics.timestamp)
                ...
                total_counts = (
                    ...
                    .filter(twelve_months_ago <= MonthlyTotalUsersMetrics.timestamp)
                ...
                unique_counts = (
                    ...
                    .filter(twelve_months_ago <= DailyUniqueUsersMetrics.timestamp)
                    .filter(DailyUniqueUsersMetrics.timestamp < today)
                    .group_by(func.date_trunc(bucket_size, DailyUniqueUsersMetrics.timestamp))
                    .all()
                )
                ...
                total_counts = (
                    ...
                    .filter(twelve_months_ago <= DailyTotalUsersMetrics.timestamp)
                    .filter(DailyTotalUsersMetrics.timestamp < today)
                    .group_by(func.date_trunc(bucket_size, DailyTotalUsersMetrics.timestamp))
                    .all()
                )
